...
describe('Forum', () => {
  ...
  beforeEach(async () => {
    ...
    getFlowAddress = await Goflow.getAddress();
    const Forum = await ethers.deployContract('Forum',[getFlowAddress]);
  await Forum.waitForDeployment();
  forumAddress = await Forum.getAddress();
   goflow = new ethers.Contract(getFlowAddress,goflowAbi.abi,owner);
   forum = new ethers.Contract(forumAddress,forumAbi.abi,owner);
  });

  describe('Deployment', () => {
    it('should deploy the Forum contract and import Goflow Token', async () => {
      console.log('Forum address: ', forumAddress);
      console.log('imported Goflow address: ', getFlowAddress);
